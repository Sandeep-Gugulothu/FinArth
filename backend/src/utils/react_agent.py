...
async def generate_insight_with_reason_act(query: str, user_id: Optional[int] = None) -> ReActResult:
...
Action: [tool_name] (if needed, otherwise say "none")"""
        ...
        thought_response = opik_client.chat.completions.create(
            model=MODEL_NAME,
        ...
        if action and action != "none" and action in mock_tools:
            logger.debug(f'Executing tool: {action}', user_id, {'step': i + 1, 'tool': action})
            step.action = action
            tool_result = await mock_tools[action]()
...
Focus on actionable insights and specific recommendations."""
    ...
    final_response = opik_client.chat.completions.create(
        model=MODEL_NAME,
